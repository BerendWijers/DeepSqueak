function  Calls=SqueakDetect(inputfile,networkfile,fname,Settings,currentFile,totalFiles,networkname)
    ...
    n_workers = 11;
    disp(['Requested pool size: ', num2str(n_workers)])
    % Return the current pool information. If there is none, returns 0x0
    cur_pool = gcp('nocreate');
    % If there is no pool, create the pool with the correct amount of workers
    if isempty(gcp('nocreate'));
        % No pool, create pool with n_workers
        disp(['Creating pool with: ', num2str(n_workers), ' workers'])
        myCluster = parcluster('local');
        myCluster.NumWorkers = n_workers;
        saveProfile(myCluster);
        parpool('local',n_workers);
        %parpool('local',n_workers);
    % Check if the number of workers of the current pool matches the requested
    elseif cur_pool.NumWorkers ~= n_workers;
        disp('Current pool did not have requested amount of workers')
        % Was not the requested pool size, delete the pool
        delete(gcp('nocreate'))
        disp(['Creating pool with: ', num2str(n_workers), ' workers'])
        myCluster = parcluster('local');
        myCluster.NumWorkers = n_workers;
    ...
    %for i = 1:length(chunks)-1
        try
            ...
            % Detect!
            [bboxes, scores, Class] = detect(network, im2uint8(im), 'ExecutionEnvironment','gpu','SelectStrongest',1); % added explicit GPU and mex Accel.
