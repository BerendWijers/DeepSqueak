...
function multinetdect_Callback(hObject, eventdata, handles, SingleDetect)
    ...
    for j = 1:length(audioselections)
        ...
        if isempty(Calls)
            ...
            disp(['Moving: ',audioname, ' to: ', processed_dir])
            src_file = AudioFile;
            % Renamed to a b c to avoid conflict with existing vars such as fname
            [a,b,c] = fileparts(AudioFile); % a = fpath, b = fname, c = ext. 
            dest_file = [processed_dir,b,c];
